import unittest
from closest_pair_problem import check_input_data_format


class InputTest(unittest.TestCase):

    def test_input_with_string_started_with_zero(self):
    ...
    def test_input_with_wrong_int_value(self):
        with self.assertRaises(TypeError) as context:
            self.assertRaises(TypeError, check_input_data_format('1'))
        exception_message = str(context.exception)
        self.assertEqual('Invalid data format', exception_message)
